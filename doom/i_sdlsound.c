...
static boolean
ExpandSoundData_SRC (sfxinfo_t *sfxinfo, byte *data, int samplerate,
                     int length)
{
    SRC_DATA src_data;
    ...
    for (i = 0; i < src_data.output_frames_gen; ++i)
        {
            ...
            // gradually overtaken by noise, with the loudest parts going
            // first. However, a moderate amount of clipping is often tolerated
            // in the quest for the loudest possible sound overall.  The
            // results of using INT16_MAX as the multiplier are not all that
            // bad, but artifacts are noticeable during the loudest parts.
...
static boolean
ExpandSoundData_SDL (sfxinfo_t *sfxinfo, byte *data, int samplerate,
                     int length)
{
    ...
    if (use_libsamplerate != 0)
        {
            if (SRC_ConversionMode () < 0)
                {
                    I_Error ("I_SDL_InitSound: Invalid value for "
                             "use_libsamplerate: %i",
                             use_libsamplerate);
                }
