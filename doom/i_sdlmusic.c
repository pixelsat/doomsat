...
ParseVorbisComment (file_metadata_t *metadata, char *comment)
{
    ...
    *eq = '\0';
    ...
    if (!strcmp (key, LOOP_START_TAG))
        {
            metadata->start_time
                = ParseVorbisTime (metadata->samplerate_hz, value);
        }
...
LoadSubstituteConfigs (void)
{
    ...
    if (!strcmp (configdir, ""))
        ...
        {
            musicdir
                = M_StringJoin (configdir, "music", DIR_SEPARATOR_S, NULL);
        }
...
I_InitTimidityConfig (void)
{
    ...
    if (success)
        {
            env_string
                = M_StringJoin ("TIMIDITY_CFG=", temp_timidity_cfg, NULL);
            putenv (env_string);
        }
    else
        {
    ...
    if (filename != NULL)
        {
            ...
            if (music == NULL)
                ...
                {
                    // Read loop point metadata from the file so that we know
                    // where to loop the music.
    ...
    if (playing_substitute && file_metadata.valid)
        {
            ...
            // If we have reached the loop end point then we have to take
            // action.
