...
#include <allegro.h>
#undef uint32_t // ugly hack because Allegro is Old and doesn't know modern GCC
                // has stdint.h
...
void
key_callback (int scancode)
{
    // this is in interrupt context, avoid heavy processing and be careful
...
DG_DrawFrame ()
{
    ...
    if (!videomode_set)
       ...
       // does not seem to work on real DOS hardware
            ...
            // while this is also done by allegro_exit handler registered
            // earlier that's too late for ENDOOM screen to work correctly
            ...
            // replace the internal line pointers with pointers into
            // DG_ScreenBuffer this avoids some copying it might crash when
            // freeing the bitmap but we never do
