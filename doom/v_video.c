...
V_DrawPatch (int x, int y, patch_t *patch)
{
    ...
    if (x < 0 || x + SHORT (patch->width) > SCREENWIDTH || y < 0
        ...
        {
            I_Error ("Bad V_DrawPatch x=%i y=%i patch.width=%i "
                     "patch.height=%i topoffset=%i leftoffset=%i",
...
void
WritePNGfile (char *filename, byte *data, int width, int height, byte *palette)
{
    png_structp ppng;
    png_infop pinfo;
    png_colorp pcolor;
...
V_DrawMouseSpeedBox (int speed)
{
    ...
    V_DrawFilledBox (box_x, box_y, MOUSE_SPEED_BOX_WIDTH,
    ...
    if (linelen < redline_x)
    ...
    else
        {
            V_DrawHorizLine (box_x + 1, box_y + MOUSE_SPEED_BOX_HEIGHT / 2,
                             redline_x, white);
            V_DrawHorizLine (box_x + redline_x,
