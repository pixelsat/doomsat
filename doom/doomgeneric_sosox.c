...
static void
add_key_to_queue (int pressed, unsigned char key)
{
...
DG_Init ()
{
    ...
    GrSelectEvents (wid, GR_EVENT_MASK_EXPOSURE | GR_EVENT_MASK_KEY_UP
    ...
    while (GrPeekEvent (&event))
        {
            ...
            switch (event.type)
                ...
                case GR_EVENT_TYPE_KEY_DOWN:
                    // this event comes continuously while pressed, so checking
                    // for changes only!
                    kp = (GR_EVENT_KEYSTROKE *)&event;
                    key = convert_to_doom_keyx (kp->ch);
                    if (key > 0 && key < sizeof (g_key_states))
