...
void
cmap_to_rgb565 (uint16_t *out, uint8_t *in, int in_pixels)
{
    int i, j;
    struct color c;
    uint16_t r, g, b;
...
void
cmap_to_fb (uint8_t *out, uint8_t *in, int in_pixels)
{
    int i, k;
    struct color c;
    uint32_t pix;
    ...
    for (i = 0; i < in_pixels; i++)
        {
            ...
            if (s_Fb.bits_per_pixel == 16)
                {
                    // RGB565 packing
                    uint16_t p = ((c.r & 0xF8) << 8) | ((c.g & 0xFC) << 3)
                                 | (c.b >> 3);

#ifdef SYS_BIG_ENDIAN
                    p = swapeLE16 (p); // can't use SHORT() because this needs
                                       // to stay unsigned
#endif
            ...
            else
                {
                    ...
                    I_Error ("No idea how to convert %d bpp pixels",
                             s_Fb.bits_per_pixel);
                }
...
I_InitGraphics (void)
{
    ...
    printf ("I_InitGraphics: framebuffer: x_res: %d, y_res: %d, x_virtual: "
            "%d, y_virtual: %d, bpp: %d\n",
...
I_FinishUpdate (void)
{
    ...
    y_offset = (((s_Fb.yres - (SCREENHEIGHT * fb_scaling))
                 * s_Fb.bits_per_pixel / 8))
               / 2;
    x_offset = (((s_Fb.xres - (SCREENWIDTH * fb_scaling)) * s_Fb.bits_per_pixel
                 / 8))
               / 2; // XXX: siglent FB hack: /4 instead of /2, since it seems
                    // to handle the resolution in a funny way
    // x_offset     = 0;
    x_offset_end
                    ...
                    if (fb_scaling == 1)
                        {
                            memcpy (line_out, line_in,
                                    SCREENWIDTH); /* fb_width is bigger than
                                                     Doom SCREENWIDTH... */
                        }
                    else
                        {
                            ...
                            for (j = 0; j < SCREENWIDTH; j++)
                                {
                                    ...
                                    for (k = 0; k < fb_scaling; k++)
                                        {
                                            line_out[j * fb_scaling + k]
                                                = line_in[j];
                                        }
                                }
                        }
#else
                    // cmap_to_rgb565((void*)line_out, (void*)line_in,
                    // SCREENWIDTH);
                    cmap_to_fb ((void *)line_out, (void *)line_in,
                                SCREENWIDTH);
#endif
                    line_out += (SCREENWIDTH * fb_scaling
                                 * (s_Fb.bits_per_pixel / 8))
                                + x_offset_end;
                }
            line_in += SCREENWIDTH;
...
void
I_SetWindowTitle (char *title)
{
    DG_SetWindowTitle (title);
}

void
I_GraphicsCheckCommandLine (void)
{
}
