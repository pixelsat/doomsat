...
static void
DonutOverrun (fixed_t *s3_floorheight, short *s3_floorpic, line_t *line,
    ...
    if (first)
        {
            ...
            if (p > 0)
                {
                    // Dump of needed memory: (fixed_t)0000:0000 and
                    // (short)0000:0008
                    //
                    // C:\>debug
                    // -d 0:0
                    ...
                    if (tmp_s3_floorpic >= numflats)
                        {
                            fprintf (stderr,
                                     "DonutOverrun: The second parameter for "
                                     "\"-donut\" "
                                     "switch should be greater than 0 and "
                                     "less than number "
                                     "of flats (%d). Using default value (%d) "
                                     "instead. \n",
                    ...
                    if (s3 == NULL)
                        {
                            ...
                            fprintf (stderr, "EV_DoDonut: WARNING: emulating "
                                             "buffer overrun due to "
                                             "NULL back sector. "
                                             "Unexpected behavior may occur "
                                             "in Vanilla Doom.\n");

                            DonutOverrun (&s3_floorheight, &s3_floorpic, line,
                                          s1);
                        }
                    else
                        {
