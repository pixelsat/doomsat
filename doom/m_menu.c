...
M_DrawSaveLoadBorder (int x, int y)
{
    ...
    for (i = 0; i < 24; i++)
        {
            V_DrawPatchDirect (
                x, y + 7, W_CacheLumpName (DEH_String ("M_LSCNTR"), PU_CACHE));
            x += 8;
        }
...
M_DrawThermo (int x, int y, int thermWidth, int thermDot)
{
    ...
    for (i = 0; i < thermWidth; i++)
        {
            V_DrawPatchDirect (
                xx, y, W_CacheLumpName (DEH_String ("M_THERMM"), PU_CACHE));
            xx += 8;
        }
    V_DrawPatchDirect (xx, y,
    ...
    if (saveStringEnter)
        {
            switch (key)
                ...
                default:
                    ...
                    // entering a savegame name. If vanilla_keyboard_mapping is
                    // on, we want to emulate this bug by using 'data1'. But if
                    // it's turned off, it implies the user doesn't care about
                    // Vanilla emulation: just use the correct 'data2'.
