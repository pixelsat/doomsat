...
static void
I_Allegro_SetMusicVolume (int volume)
{
    int digivol = -1;
...
// Start playing a mid
static void
I_Allegro_PlaySong (void *handle, boolean looping)
{
    int retval;
...
static void
I_Allegro_UnRegisterSong (void *handle)
{
...
static void *
I_Allegro_RegisterSong (void *data, int len)
{
    char *filename;
    MIDI *music;
    ...
    // Load the MIDI. In an ideal world we'd load it directly from memory but
    // Allegro doesn't support that so we have to generate a temporary file.
