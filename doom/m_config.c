...
static int
ParseIntParameter (char *strparm)
{
    int parm;
...
static void
SetVariable (default_t *def, char *value)
{
    int intparm;
...
void
M_BindVariable (char *name, void *location)
{
    default_t *variable;
...
M_GetSaveGameDir (char *iwadname)
{
    ...
    if (!strcmp (configdir, ""))
    ...
    else
        {
            ...
            free (topdir);
#else
            savegamedir = M_StringJoin (configdir, DIR_SEPARATOR_S,
                                        ".savegame/", NULL);
