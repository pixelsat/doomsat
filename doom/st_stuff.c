    ...
    if (st_statusbaron)
        {
            ...
            V_CopyRect (ST_X, 0, st_backing_screen, ST_WIDTH, ST_HEIGHT, ST_X,
                        ST_Y);
        }
}
            ...
            if (!netgame && gameskill != sk_nightmare)
                {
                    ...
                    // 'mus' cheat for changing music
                    else if (cht_CheckCheat (&cheat_mus, ev->data2))
                        {
                            ...
                            // Note: The original v1.9 had a bug that tried to
                            // play back the Doom II music regardless of
                            // gamemode.  This was fixed in the Ultimate Doom
                            // executable so that it would work for the Doom 1
                            // music as well.

                            if (gamemode == commercial
                                || gameversion < exe_ultimate)
                                {
                                    musnum = mus_runnin + (buf[0] - '0') * 10
                                             + buf[1] - '0' - 1;

                                    if (((buf[0] - '0') * 10 + buf[1] - '0')
                                        > 35)
                                        plyr->message
                        ...
                        {
                            // Noclip cheat.
                            // For Doom 1, use the idspipsopd cheat; for all
                            // others, use idclip
            ...
            if (!netgame && cht_CheckCheat (&cheat_clev, ev->data2))
                {
                    ...
                    // Vanilla Doom allows IDCLEV up to MAP40 even though it
                    // normally crashes.
...
static void
ST_loadCallback (char *lumpname, patch_t **variable)
{
...
static void
ST_unloadCallback (char *lumpname, patch_t **variable)
{
