...
static int fbFd;
static unsigned int fbWidth, fbHeight, fbStride, fbBytesPerPixel, fbOffsetX,
    fbOffsetY;
                    ...
                    if (pollfds[i].revents & POLLIN)
                        {
                            ...
                            keepGoing
                                = true; // we read something, so keep trying
...
DG_Init ()
{
    ...
    if (numInputFds == 0)
        I_Error ("Failed to find any compatible input device, see the logs "
                 "above for potential problems");
...
DG_DrawFrame ()
{
    ...
    for (int line = 0; line < DOOMGENERIC_RESY; line++)
        {
            memcpy ((void *)((uintptr_t)(fbPtr) + (fbStride * line) + fbOffsetY
                             + fbOffsetX),
