...
static void
I_Allegro_PrecacheSounds (sfxinfo_t *sounds, int num_sounds)
{
    char namebuf[9];
    int i;
...
static int
I_Allegro_GetSfxLumpNum (sfxinfo_t *sfx)
{
    char namebuf[9];
...
static void
I_Allegro_UpdateSoundParams (int handle, int vol, int sep)
{
    int left, right;
...
static int
I_Allegro_StartSound (sfxinfo_t *sfxinfo, int channel, int vol, int sep)
{
...
static void
I_Allegro_StopSound (int handle)
{
    if (!sound_initialized || handle < 0 || handle >= NUM_CHANNELS)
        {
            return;
...
static boolean
I_Allegro_SoundIsPlaying (int handle)
{
    int position;
...
static boolean
I_Allegro_InitSound (boolean _use_sfx_prefix)
{
    int i;
